...
const toHandlerKey = cacheStringFunction$1(str => str ? `on${capitalize(str)}` : ``); // compare whether a value has changed, accounting for NaN.
...
function renderComponentRoot(instance) {
  ...
  try {
    ...
    if (false && isCompatEnabled$1("INSTANCE_ATTRS_CLASS_STYLE"
...
class VueElement extends BaseClass {
  ...
  _resolveDef() {
    ...
    const resolve = def => {
      this._resolved = true; // check if there are props set pre-upgrade or connect
      ...
      const {
        props,
        styles
      } = def; // defining getter/setters on prototype
...
const IS_MEMO_SAME = Symbol(`isMemoSame` ); // Name mapping for runtime helpers that need to be imported from 'vue' in
...
function injectProp(node, prop, context) {
  ...
  const originalProps = node.type === 13
  /* VNODE_CALL */
      ...
      && s[0] === '<') {
        ...
        if (s.length === 1) {
          ...
          && isSpecialTemplateDirective(p.name))) {
            warnDeprecation("COMPILER_NATIVE_TEMPLATE"
            /* COMPILER_NATIVE_TEMPLATE */
            , context, node.loc);
            node = node.children;
          }
...
function createChildrenCodegenNode(branch, keyIndex, context) {
  ...
  if (needFragmentWrapper) {
    ...
    /* FOR */
      ...
      return createVNodeCall(context, helper(FRAGMENT), createObjectExpression([keyProperty]), children, patchFlag + (` /* ${patchFlagText} */` ), undefined, undefined, true, false, false
      /* isComponent */
      , branch.loc);
    }
  } else {
...
function getBaseTransformPreset(prefixIdentifiers) {
  return [[transformOnce, transformIf, transformMemo, transformFor, ...[transformFilter], ...([transformExpression] ), transformSlotOutlet, transformElement, trackSlotScopes, transformText], {
    on: transformOn$1,
    bind: transformBind,
...
const transformModel = (dir, node, context) => {
  const baseResult = transformModel$1(dir, node, context); // base transform has errors OR component v-model (only need props)
...
script$1.__file = "src/js/Test.vue";
...
script.__file = "src/js/App.vue";

console.log('main'); // https://github.com/kutia-software-company/vue3-starter
